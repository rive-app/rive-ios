...
class AudioTests: XCTestCase {
    ...
    @MainActor
    func test_decodeAudio_whenNoHandleIsCreated_doesNotDeleteHandle() async {
        let commandQueue = MockCommandQueue()
        var service: AudioService? = AudioService(dependencies: .init(commandQueue: commandQueue, messageGate: CommandQueueMessageGate(driver: commandQueue)))
        weak var releasedService = service
        commandQueue.stubDecodeAudio { _, listener, requestID in
            listener.onAudioSourceError(0, requestID: requestID, message: "Allocation failed")
            return 0
        }
        do {
            _ = try await service!.decodeAudio(from: Data())
            XCTFail("Expected AudioError.failedDecoding")
        } catch AudioError.failedDecoding {
        } catch {
            XCTFail("Expected AudioError.failedDecoding, got \(error)")
        }
    ...
    @MainActor
    func test_decodeAudio_whenCopyThrowsBadAlloc_throwsError() async throws {
        try await assertCopyError(.badAlloc)
    }

    @MainActor
    func test_decodeAudio_whenCopyThrowsLengthError_throwsError() async throws {
        try await assertCopyError(.lengthError)
    }

    @MainActor
    func test_decodeAudio_withValidData_succeeds() async throws {
        let worker = try await Worker()
        let url = try XCTUnwrap(Bundle(for: Self.self).url(
            forResource: "racket1-59343", withExtension: "wav"
        ))
        let validData = try Data(contentsOf: url)
        let asset = try await worker.decodeAudio(from: validData)
    ...
    private func assertCopyError(_ failure: AssetCopyFailure) async throws {
        ...
        let task = Task { @MainActor in
            ...
            do {
                _ = try await worker.decodeAudio(from: validData)
                XCTFail("Expected AudioError.failedDecoding")
            } catch AudioError.failedDecoding {
            } catch {
                XCTFail("Expected AudioError.failedDecoding, got \(error)")
